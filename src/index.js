...
export default {
  async fetch(request, env) {
    ...
    //
    //    Ride alerts ('ride_alerts' channel) are sent as a DATA-ONLY
    //    message (no top-level "notification" key) on purpose: that is
    //    the only way Android guarantees delivery to the app's own
    //    RideAlertFcmService.onMessageReceived() even when the app has
    //    been fully killed, which is what lets the app show its own
    //    full-screen ringing UI (like an incoming call) instead of a
    //    plain system notification. Order/delivery alerts are untouched
    //    — they keep using a normal notification payload, which Android
    //    auto-displays via the system tray even when killed.
    const sendResults = await Promise.all(tokens.map(async (token) => {
      const isRideAlert = channelId === 'ride_alerts';
      const message = isRideAlert
        ? {
            token,
...
async function runEscalationSweep(env) {
  ...
  await Promise.all(jobs.map(async (job) => {
    const isRideAlert = job.channelId === 'ride_alerts';
    const message = isRideAlert
